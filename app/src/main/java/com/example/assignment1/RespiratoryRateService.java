...
public class RespiratoryRateService extends Service implements SensorEventListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        sen_acc = manager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        manager.registerListener(this, sen_acc, SensorManager.SENSOR_DELAY_NORMAL);
    ...
    private int getRespiratoryRate() {
        ...
        List<Double> x_avg = a.calcMovingAvg(mov_period, x);
        int peaksX = a.countZerosThreshold(x_avg);

        List<Double> y_avg = a.calcMovingAvg(mov_period, y);
        int peaksY = a.countZerosThreshold(y_avg);

        List<Double> z_avg = a.calcMovingAvg(mov_period, z);
        int peaksZ = a.countZerosThreshold(z_avg);
